...
class Client(object):
    ...
    def __init__(self, client_id, client_secret, site='MLM'):
    ...
    @valid_token
    def get_orders(self,sellerid,offset:None):
        """Returns account information about the authenticated user.
        ...
        if offset==None:
            offset=0
        return self._get('/orders/search?seller={}&offset={}'.format(sellerid,offset))
 
 
 
    @valid_token
    def update_orders(self,sellerid,offset:None):
        """Returns account information about the authenticated user.
        ...
        if offset==None:
            offset=0
        return self._get('/orders/search?seller={}&offset={}&sort=date_desc'.format(sellerid,offset))
    ...
    @valid_token
    def get_info_users(self, site_id,username):
        """Returns information about listing types.
        ...
        Returns:
            A dict.
        """
        return self._get('/sites/{}/search?nickname={}'.format(site_id,username))
    ...
    def _request(self, method, endpoint, params=None, **kwargs):
        ...
        response = requests.request(method, self.BASE_URL + endpoint, json=_params, **kwargs)
         
        if 'Error from cloudfront' in str(response.headers):
            print("falla")
            print(_params)
